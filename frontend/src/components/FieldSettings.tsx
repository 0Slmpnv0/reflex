...
export default function FieldSettings({
  ...
  return (
    <div className="field-setting">
      <textarea name={name} id={id} rows={1}>{name}</textarea>
      ...
      >
        {
          currentType === 'unselected' ? (
            <option value="unselected" hidden selected>unselected</option>
          ) :
          (
            <option value="unselected" hidden>unselected</option>
          )
        }
        {
          possible_field_types.map( 
            (fieldType: string) => (
              fieldType === currentType ? (
                <option key={fieldType} value={currentType} selected>{currentType}</option>
              ) : (
